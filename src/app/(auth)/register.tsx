    ...
    <Pressable
      ...
      className={[
        ...
        selected
          ...
          : 'border-neutral-200 dark:border-dark-border bg-surface-raised dark:bg-dark-card',
      ].join(' ')}
    >
      <View className={[
        'w-14 h-14 rounded-full items-center justify-center',
        selected ? 'bg-sky-100 dark:bg-sky-900' : 'bg-neutral-100 dark:bg-dark-control',
      ].join(' ')}>
        <Icon
          name={isExpert ? 'Stethoscope' : 'User'}
          size={28}
          color={selected ? '#0EA5E9' : (isDark ? '#A3A3A3' : '#737373')}
        />
      </View>
          ...
          {([1, 2] as const).map((s) => (
            <View
              ...
              className={[
                ...
                s <= step ? 'bg-sky-500' : 'bg-neutral-200 dark:bg-dark-control',
              ].join(' ')}
            />
          ))}
        </View>
